		...
		const struct i2c_device_id *id)
{
	...
	if (ret) {
		pr_info("Could not set to 2800mv.\n");
	}
	...
	if (ret) {
		pr_err("Could not set to 1800mv.\n");
	}
#endif /* CONFIG_ENABLE_REGULATOR_POWER_ON */
...
static const struct of_device_id touch_match_table[] = {
...
static int __init touch_driver_init(void)
{
	...
	if (ret < 0) {
		pr_info("add MStar touch device driver i2c driver failed.\n");
		return -ENODEV;
	}
	pr_info("add MStar touch device driver i2c driver.\n");
...
{
	pr_info("remove MStar touch device driver i2c driver.\n");
