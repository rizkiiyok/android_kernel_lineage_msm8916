...
extern const struct of_device_id touch_dt_match_table[];
...
static long _DrvPlatformLyrProximityIoctl(struct file *file, unsigned int cmd,
		unsigned long arg)
{
#if 0
...
static struct device_attribute proximity_attribute = __ATTR(proximity, 0666/*0664*/,
		_DrvPlatformLyrProximityDetectionShow, _DrvPlatformLyrProximityDetectionStore);
...
static int _DrvPlatformLyrProximityInputDeviceInit(struct i2c_client *pClient)
{
	...
	if (nRetVal < 0) {
		DBG(&g_I2cClient->dev,
		    "*** Failed to sysfs_create_group() for proximity *** nRetVal=%d\n", nRetVal);
	...
	if (nRetVal < 0) {
		DBG(&g_I2cClient->dev,
		    "*** Failed to sensors_classdev_register() for proximity *** nRetVal=%d\n", nRetVal);
...
static void _DrvPlatformLyrVirtualKeysInit(void)
{
	...
	if (g_PropertiesKObj == NULL) {
		DBG(&g_I2cClient->dev,
		    "*** Failed to kobject_create_and_add() for virtual keys *** nRetVal=%d\n", nRetVal);
		return;
	}
	...
	if (nRetVal < 0) {
		DBG(&g_I2cClient->dev,
		    "*** Failed to sysfs_create_group() for virtual keys *** nRetVal=%d\n", nRetVal);
...
static s32 _DrvPlatformLyrTouchPinCtrlInit(struct i2c_client *pClient)
{
	...
	_gGpioRst = of_get_named_gpio(pDeviceNode, "mstar,reset-gpio", 0);
	...
	if (!gpio_is_valid(_gGpioRst) || !gpio_is_valid(_gGpioIrq)) {
		DBG(&g_I2cClient->dev, "Invild _gGpioRst = %d, _gGpioIrq = %d\n", _gGpioRst, _gGpioIrq);
		return -EINVAL;
	}
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(bool nFlag)
{
	...
	if (g_ReguVdd != NULL && g_ReguVcc_i2c != NULL) {
		if (nFlag == true) {
			...
			if (nRetVal) {
				DBG(&g_I2cClient->dev, "enable(g_ReguVdd) failed. nRetVal=%d\n", nRetVal);
			}
			msleep(20);

			nRetVal = regulator_enable(g_ReguVcc_i2c);
			if (nRetVal) {
				DBG(&g_I2cClient->dev, "enable(g_ReguVcc_i2c) failed. nRetVal=%d\n", nRetVal);
			}
			msleep(20);
		} else {
			nRetVal = regulator_disable(g_ReguVdd);
			if (nRetVal) {
				DBG(&g_I2cClient->dev, "disable(g_ReguVdd) failed. nRetVal=%d\n", nRetVal);
			}
			msleep(20);

			nRetVal = regulator_disable(g_ReguVcc_i2c);
			if (nRetVal) {
				DBG(&g_I2cClient->dev, "disable (g_ReguVcc_i2c) failed. nRetVal=%d\n", nRetVal);
			}
			msleep(20);
		}
	}
...
/* For specific MTK BB chip(ex. MT6735), need to enable this function call for correctly power on Touch IC. */
#endif /* CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD */
...
void DrvPlatformLyrTouchDevicePowerOn(void)
{
	...
	usleep_range(10000, 10500);  /* reset pulse */
	...
	g_IsHwResetByDriver =
		1; /* Indicate HwReset is triggered by Device Driver instead of Firmware or Touch IC */
#endif /* CONFIG_ENABLE_ESD_PROTECTION */
}
...
void DrvPlatformLyrFingerTouchPressed(s32 nX, s32 nY, s32 nPressure, s32 nId)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX ||
		...
		input_report_abs(g_InputDevice, ABS_MT_TRACKING_ID,
				 nId); /* ABS_MT_TRACKING_ID is used for MSG26xxM/MSG28xx only */
	}
...
s32 DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
	...
	*/
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX ||
	    g_ChipType == CHIP_TYPE_MSG58XXA) {
		input_set_abs_params(g_InputDevice, ABS_MT_TRACKING_ID, 0, (MUTUAL_MAX_TOUCH_NUM - 1), 0,
				     0); /* ABS_MT_TRACKING_ID is used for MSG26xxM/MSG28xx only */
	}
...
#endif /* CONFIG_ENABLE_TYPE_B_PROTOCOL */
	input_set_abs_params(g_InputDevice, ABS_MT_POSITION_X, TOUCH_SCREEN_X_MIN,
			     TOUCH_SCREEN_X_MAX, 0, 0);
	input_set_abs_params(g_InputDevice, ABS_MT_POSITION_Y, TOUCH_SCREEN_Y_MIN,
...
#ifdef CONFIG_ENABLE_TOUCH_PIN_CONTROL
	nRetVal = _DrvPlatformLyrTouchPinCtrlInit(pClient);
	if (nRetVal != 0)
		return nRetVal;
#endif /* CONFIG_ENABLE_TOUCH_PIN_CONTROL */
	...
	if (DrvIcFwLyrIsRegisterFingerTouchInterruptHandler()) {
...
#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
		{
			...
			if (pDeviceNode) {
				...
				DBG(&g_I2cClient->dev, "request_irq() find touch device node failed\n");
			}
		...
		mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM,
				     ...
				     _DrvPlatformLyrFingerTouchInterruptHandler, 1);
...
void DrvPlatformLyrEsdCheck(struct work_struct *pWork)
{
	...
	if (IS_FIRMWARE_DATA_LOG_ENABLED) {
		if (g_FirmwareMode ==
		    FIRMWARE_MODE_DEBUG_MODE) { /* Skip ESD check while firmware mode is DEBUG MODE */
			DBG(&g_I2cClient->dev, "Not allow to do ESD check while firmware mode is DEBUG MODE.\n");
			goto EsdCheckEnd;
		}
	}
/* Skip ESD check while mp test is triggered by MTPTool APK through JNI interface */
 ...
 /* Method 1. Require the new ESD check command(CmdId:0x55) support from firmware which is
 currently implemented for MSG22XX only. So default is not supported. */
#ifdef CONFIG_ENABLE_ESD_CHECK_COMMAND_BY_FIRMWARE
	szTxData[0] = 0x55;
	szTxData[1] = 0xAA;
	szTxData[2] = 0x55;
	...
	DBG(&g_I2cClient->dev, "szRxData[] : 0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x  0x%x\n",
	    szRxData[0], szRxData[1], szRxData[2], szRxData[3], szRxData[4], szRxData[5], szRxData[6],
	    szRxData[7]);
	...
	if (retW > 0 && retR > 0) {
		...
		if (i == 8) {
			if (szRxData[0] == 0 && szRxData[1] == 0 && szRxData[2] == 0 && szRxData[3] == 0 &&
			    szRxData[4] == 0 && szRxData[5] == 0 && szRxData[6] == 0 && szRxData[7] == 0) {
	...
	while (i < 3) {
		...
		if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX ||
		    g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX ||
		    g_ChipType == CHIP_TYPE_MSG58XXA) {
