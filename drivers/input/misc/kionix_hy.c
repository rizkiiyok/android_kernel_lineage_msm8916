...
/* ensure that PC1 is cleared before updating control registers */
	...
	if ((acceld->accel_drdy==1)&&(acceld->client->irq)) {
		err = i2c_smbus_write_byte_data(acceld->client,
		...
		int axis, int apply_now)
{
	...
	for(i = 0; i<CAL_COUNT; i++)		//Get multi-group data for average
	{
		...
		printk("axis=%d, acceld->accel_data[acceld->axis_map_x]=%d, acceld->accel_data[acceld->axis_map_y]=%d, acceld->accel_data[acceld->axis_map_z]=%d\n", axis,
	...
	sum_z = sum_z/CAL_COUNT;
      printk("sum_x=%d, sum_y=%d, sum_z=%d\n",
				sum_x, sum_y, sum_z);
	//X, Y, Z offset
	acceld->accel_cali[acceld->axis_map_x] = -sum_x;
	acceld->accel_cali[acceld->axis_map_y] = -sum_y;
	acceld->accel_cali[acceld->axis_map_z] = -sum_z + 16384;
      printk("acceld->accel_cali[acceld->axis_map_x] =%d, acceld->accel_cali[acceld->axis_map_y]=%d, acceld->accel_cali[acceld->axis_map_z]=%d\n",
